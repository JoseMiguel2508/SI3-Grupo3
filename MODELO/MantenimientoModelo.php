...
class MantenimientoModelo {
    ...
    public function registrarMantenimiento($id_vehiculo, $tipo, $descripcion, $costo, $fecha_servicio, $fecha_proximo, $estado, $notas, $creado_por) {
        $sql = "INSERT INTO registros_mantenimiento (id_vehiculo, tipo_mantenimiento, descripcion, costo, fecha_servicio, fecha_proximo_servicio, estado, notas, creado_por) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $this->conexion->prepare($sql);
        $stmt->bind_param("issdssssi", $id_vehiculo, $tipo, $descripcion, $costo, $fecha_servicio, $fecha_proximo, $estado, $notas, $creado_por);
        return $stmt->execute();
    }
